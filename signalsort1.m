function  [maxorg,minorg,index,ss]=signalsort1(signals,imgrgb,N,slectindex,r)
Nn=size(signals,1);
...
%         if psnr(nnsimg1,imgrgb1) > psnr(ssimg1,imgrgb1)
        if ssim(nnsimg1,imgrgb1) > ssim(ssimg1,imgrgb1)
            ss(j)=1;
%             disp('1')
            ssimg1 = nnsimg1;
        else
...
%         tssim(j) = psnr(ssimg1,imgrgb1);  
        tssim(j) = ssim(ssimg1,imgrgb1);
        % %------------全局评价-------------%
...
%     imgrgb1=imgrgb([1:10 medw-4:medw+5 width-9:width],[1:10 medh-4:medh+5 height-9:height]);
    ...
    for j = 1:1:N
        ...
        nnsimg = reshape(ns,width,height);
        %-----------局部评价---------------%
%         nnsimg1=nnsimg([1:10 medw-4:medw+5 width-9:width],[1:10 medh-4:medh+5 height-9:height]);
        nnsimg1=nnsimg([1:l medw-l1:medw+l2 width-l+1:width],[1:l medh-l1:medh+l2 height-l+1:height]);
%         ssimg1=ssimg([1:10 medw-4:medw+5 width-9:width],[1:10 medh-4:medh+5 height-9:height]);
